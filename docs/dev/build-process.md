# Build Process

This document describes what happens when you run `mvn clean install` and the artifacts produced at
each stage.

## Quick Reference

```bash
# Full build (clean + compile + test + package + integration tests + install)
mvn clean install

# Compile only
mvn compile

# Run unit tests
mvn test

# Package artifacts (JAR, uber JAR, DEB, RPM)
mvn package

# Full validation including integration and E2E tests
mvn verify
```

## Build Phases

### 1. Clean

Deletes the `target/` directory and all previously built artifacts.

### 2. Compile

**Plugin:** `maven-compiler-plugin`

Compiles all Java sources under `src/main/java` using Java 25 language level. The `--release 25`
flag ensures the compiled bytecode is compatible with Java 25 and later.

### 3. Test (unit tests)

**Plugin:** `maven-surefire-plugin`

Runs unit tests under `src/test/java`. These tests use JUnit (Jupiter), Mockito, and AssertJ to
verify individual command behavior against mocked `MBeanServerConnection` instances.

> Tests in the `sh.jmx.jmxsh.jdk*` package are excluded because they depend on
> platform-specific JVM attach APIs.

### 4. Package

Several plugins run during the package phase to produce the distribution artifacts:

#### 4a. JAR

**Plugin:** `maven-jar-plugin`

Produces the standard library JAR (`jmxsh-<version>.jar`) containing only the project's compiled
classes.

#### 4b. Uber JAR

**Plugin:** `maven-shade-plugin`

Produces a self-contained uber JAR (`jmxsh-<version>-uber.jar`) that bundles all runtime
dependencies into a single executable file. The shade plugin handles `META-INF/services` merging
and license aggregation via resource transformers. The manifest sets
`sh.jmx.jmxsh.boot.CliMain` as the main class, so it can be run directly:

```bash
java -jar jmxsh-<version>-uber.jar
```

#### 4c. Debian Package

**Plugin:** `jdeb`

Produces a `.deb` package for Debian/Ubuntu systems. Installs the uber JAR to
`/usr/share/jmxsh/jmxsh-uber.jar` and a launch script to `/usr/bin/jmxsh`.

#### 4d. RPM Package

**Plugin:** `rpm-maven-plugin`

Produces an `.rpm` package for Red Hat/CentOS/Fedora systems with the same installation layout as
the DEB package.

#### 4e. Source JAR

**Plugin:** `maven-source-plugin`

Attaches a source JAR (`jmxsh-<version>-sources.jar`) for distribution alongside the binary.

### 5. Verify (integration + E2E tests)

**Plugin:** `maven-failsafe-plugin`

Runs integration and end-to-end tests (files matching `*IT.java`):

- **Integration tests** (`src/test/java/.../integration/`) — exercise commands against an embedded
  JMX server running in the same JVM
- **E2E tests** (`src/test/java/.../e2e/`) — launch the uber JAR as a subprocess and communicate
  with a separate target JVM over JMX/RMI

### 6. Install

Copies all artifacts (JARs, DEB, RPM) into the local Maven repository (`~/.m2/repository`) so other
local projects can reference them as dependencies.

## Artifacts Summary

| Artifact | Location | Description |
|---|---|---|
| `jmxsh-<v>.jar` | `target/` | Library JAR (classes only) |
| `jmxsh-<v>-uber.jar` | `target/` | Executable uber JAR (all deps bundled) |
| `jmxsh-<v>.deb` | `target/` | Debian package |
| `jmxsh-<v>.rpm` | `target/rpm/` | RPM package |
| `jmxsh-<v>-sources.jar` | `target/` | Source code JAR |

## CI/CD

The CI workflow (`java-ci.yaml`) runs `mvn verify` (compile + unit + integration + E2E tests) on
every push and pull request targeting `master`.

### Releasing

Releases are created via the `release.yml` workflow, triggered manually from the GitHub Actions UI
(`workflow_dispatch`). The workflow takes a single `bump` input (`patch`, `minor`, or `major`) that
determines the release version by incrementing the corresponding component of the current POM version.
The next development version is always the release version incremented by a patch.

For example, if the POM version is `1.1.0-SNAPSHOT`:

| Bump type | Release version | Next development version |
|---|---|---|
| `patch` | `1.1.1` | `1.1.2-SNAPSHOT` |
| `minor` | `1.2.0` | `1.2.1-SNAPSHOT` |
| `major` | `2.0.0` | `2.0.1-SNAPSHOT` |

The release process:

1. **Prepare** — `mvn release:prepare` updates the POM version, generates the changelog via
   [git-cliff](https://git-cliff.org/) (`exec:exec` runs `scripts/generate-changelog.sh`), commits,
   and creates a `v<version>` tag (all locally, pushed only after a successful build)
2. **Build** — checks out the release tag and runs `mvn clean package` on JDK 25
3. **Package** — renames the uber JAR to `jmxsh-<version>.jar` (without the `-uber` suffix), then
   builds DEB and RPM packages from the uber JAR
4. **Publish** — pushes the release commits and tag, then creates a GitHub Release with the JAR, DEB,
   RPM packages, and release notes generated by git-cliff
5. **Update apt repository** — copies the DEB into `docs/apt/pool/main/`, regenerates the
   `Packages` and `Release` indexes with `apt-ftparchive`, signs them with the repository GPG key,
   and commits the result to `docs/apt/` (served at [jmx.sh/apt](https://jmx.sh/apt) via GitHub
   Pages)
6. **Update Homebrew tap** — sends a `repository_dispatch` event to
   [`nyg/homebrew-jmxsh`](https://github.com/nyg/homebrew-jmxsh) with the release version and Java
   version. This triggers an automated workflow in the tap repository that updates the formula (URL,
   SHA256, Java dependency), opens a PR, waits for bottle CI to pass, and publishes the updated
   formula.

```mermaid
sequenceDiagram
    actor Dev as Developer
    participant RW as nyg/jmxsh<br>release.yml
    participant GH as GitHub
    participant UF as homebrew-jmxsh<br>update-formula.yml
    participant TB as homebrew-jmxsh<br>brew test-bot
    participant AP as homebrew-jmxsh<br>auto-pr-pull.yml
    participant PB as homebrew-jmxsh<br>publish.yml

    Dev->>RW: workflow_dispatch (bump type)
    activate RW
    RW->>RW: mvn release:prepare
    RW->>RW: mvn clean package
    RW->>RW: Build DEB & RPM
    RW->>GH: Push commits & tag
    RW->>GH: Create GitHub Release (JAR, DEB, RPM)
    RW->>UF: repository_dispatch (version, java_version)
    deactivate RW

    activate UF
    UF->>GH: Download JAR & compute SHA256
    UF->>UF: Update Formula/jmxsh.rb
    UF->>GH: Create PR
    deactivate UF

    GH->>TB: pull_request event
    activate TB
    TB->>TB: brew test-bot (macOS Intel, macOS ARM, Linux)
    TB->>GH: Upload bottle artifacts
    deactivate TB

    GH->>AP: workflow_run completed
    activate AP
    AP->>GH: Apply pr-pull label
    deactivate AP

    GH->>PB: pull_request_target labeled
    activate PB
    PB->>PB: brew pr-pull (embed bottles)
    PB->>GH: Push to master & delete branch
    deactivate PB
```
